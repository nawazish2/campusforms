...
- **Only the organizer can read responses.** Which is why `forms` carries a
  ...
  without being able to see one. `/status` looks up a response by `ref` and
  returns queue status only.
- **Response ids and refs are generated by the client.** A submitter can't
  read their row back, and the confirmation screen shows them their own
  reference number. The database rejects a malformed `ref`.
